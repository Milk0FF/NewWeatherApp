...
fun getWeatherInfoData(
    ...
    val call: Call<JsonObject> = retrofitAPI.getWeather("Omsk", "metric")
    ...
    call!!.enqueue(object : Callback<JsonObject?> {
        override fun onResponse(call: Call<JsonObject?>?, response: Response<JsonObject?>) {
            if(response.isSuccessful()){
                ...
                Toast.makeText(ctx, "Ошибка запроса. Попробуйте позже!", Toast.LENGTH_SHORT).show()
            }

        }
        override fun onFailure(call: Call<JsonObject?>, t: Throwable) {
            Toast.makeText(ctx, "Ошибка подключения к интернету. Попробуйте позже!", Toast.LENGTH_SHORT).show()
        }
    })
